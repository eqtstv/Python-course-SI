...
def plot_validation(y_train, y_test, y_pred):
    ...
    plt.plot(np.arange(len(y_train), len(y_train) + len(y_pred)), y_pred, 'r', label="Predicted Volume", linewidth=3)
    plt.ylabel('Normalized Volume')
    plt.xlabel('Days from date entered')
    plt.legend()
    plt.show()

def plot_prediction(df, forecast):
    plt.plot(np.arange(0, len(df)), df['Normalized_volume'].to_numpy(), 'g', label="Data")
    plt.plot(np.arange(len(df), len(df) + len(forecast)), forecast, 'r', label="Forecast")
    plt.ylabel('Normalized Volume')
    plt.xlabel('Days from date entered')
    plt.legend()
    plt.show()
...
# BTC, LTC, ETH
df = update_data('BTC', '2019-04-23')

volume_train, volume_test = split_data(df)
...
volume_data = df['Normalized_volume'].values.reshape((-1)).reshape((-1,1))
data_generator = TimeseriesGenerator(volume_data, volume_data, length=look_back, batch_size=1)
forecast = model.predict_generator(data_generator)
plot_prediction(df, forecast)
